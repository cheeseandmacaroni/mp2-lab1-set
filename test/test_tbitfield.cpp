...
{
    ASSERT_NO_THROW(TBitField bf(3u));
}
...
TEST(TBitField, can_set_bit)
{
    ...
    EXPECT_EQ(false, bf.getBit(3u));

    bf.setBit(3u);
    EXPECT_NE(false, bf.getBit(3u));
}
...
TEST(TBitField, throws_when_set_bit_with_too_large_index)
{
    ...
    ASSERT_ANY_THROW(bf.setBit(11u));
}
...
TEST(TBitField, throws_when_get_bit_with_too_large_index)
{
    ...
    ASSERT_ANY_THROW(bf.getBit(11u));
}
...
TEST(TBitField, throws_when_clear_bit_with_too_large_index)
{
    ...
    ASSERT_ANY_THROW(bf.clrBit(11u));
}
...
TEST(TBitField, can_assign_bitfields_of_equal_size)
{
    ...
    EXPECT_NE(false, bf2.getBit(0u));
    EXPECT_NE(false, bf2.getBit(1u));
}
